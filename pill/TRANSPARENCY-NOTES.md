...
Status as of this branch: **capsule shape done, per-pixel transparency blocked.**
The pill renders as a capsule over an opaque black window background, which reads
as a dark band around it. This file records what was tried, what works, what is
inert, and what crashes, so nobody re-runs the same experiments.

![current state](findings/current-state-band.png)
...
| 7 | `DwmEnableBlurBehindWindow` (empty region) + `DwmExtendFrameIntoClientArea(0)` — the classic DComp per-pixel-alpha path, used by WinUIEx `TransparentTintBackdrop` | **Crashes this app**: combase `0x80131523` during the first commit after the call. Opt-in behind `--alpha` only. Needs a dedicated investigation (WinUIEx does the same calls inside its own message loop — the difference may be message ordering or the GDI base fill timing). |
...
## Remaining paths to true transparency

1. **Debug the blur-behind crash** (#7): compare against WinUIEx's exact call
   ordering (their `WindowMessageMonitor` handles `WM_ERASEBKGND` → black fill →
   `return 1`, and re-applies DWM config on `WM_DWMCOMPOSITIONCHANGED`). The
   crash may be ordering- or timing-related rather than fundamental.
2. **UpdateLayeredWindow route**: render the pill into a 32bpp ARGB bitmap and
   push it per frame. Guaranteed per-pixel alpha, but abandons XAML rendering.
3. **Custom DComp target / island rehost**: host the XAML island on a
   self-created `WS_EX_NOREDIRECTIONBITMAP` window with a `DesktopWindowTarget`.
   Big change to the app shell; full control of the surface alpha.
4. **Accept the system look**: keep `DWMWA_CORNER_PREFERENCE = ROUND` and live
   with the ~10px system-rounded rect (no capsule, no band fight).
